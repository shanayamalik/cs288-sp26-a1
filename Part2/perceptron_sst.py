"""Perceptron model for SST-2 dataset.

This script trains a perceptron on SST-2 and generates test predictions.
...
def main():
    ...
    dev_acc = model.evaluate(
        dev_data,
        save_path="results/perceptron_sst2_dev_predictions.csv"
    )
    ...
    model.evaluate(
        test_data,
        save_path="results/perceptron_sst2_test_predictions.csv"
    )
    print("Test predictions saved to results/perceptron_sst2_test_predictions.csv")
    
    # save model weights
    model.save_weights("results/perceptron_sst2_model.json")
